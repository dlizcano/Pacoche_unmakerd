...
```{r calendar2,cache=TRUE,warning=FALSE,eval=TRUE, echo=FALSE, error=FALSE, message=FALSE, fig.height=11,fig.width=8}
    ...
    presence.per.sp<-matrix(nrow = 40, ncol = 63)
    ...
    for (i in 1:length(mat.per.sp)){
      ...
      presence.per.sp[i,]<- apply(row.per.sp[i,], 2, function(x) {x[x >= 1] <- 1; x})
      
    }
    
    
    
    naive_occu<-apply(presence.per.sp,1,sum,na.rm=T)/apply(presence.per.sp,1,function(x){length(which(!is.na(x)))}) # function from Jorge
    
    cont.per.sp$naive_occu<-naive_occu
    cont.per.sp$especie<-rownames(cont.per.sp)
    colnames(cont.per.sp)<-c("Numero_de_registros","naive_occu", "especie")
    
    # xtable(arrange(df = cont.per.sp, desc(Numero_de_registros)))
    kable(arrange(cont.per.sp, desc(cont.per.sp[,1])))
    
    # write.csv(cont.per.sp, "table/naive_occu.csv") # write table
...
plot(cov.stack)
    ...
    f.sp.occu.models <- function(sp_number){
      ...
      fm1 <- occu(~ 1 ~ 1, sp_UMF, method="SANN") 
      fm2 <- occu(~ elev ~ elev, sp_UMF, method="SANN")
      fm3 <- occu(~ elev ~ slope, sp_UMF, method="SANN")
      fm4 <- occu(~ elev ~ dist_def, sp_UMF, method="SANN")
      fm5 <- occu(~ elev ~ dist_rd, sp_UMF, method="SANN")
      fm6 <- occu(~ elev ~ canopy_h, sp_UMF, method="SANN")
      fm7 <- occu(~ elev ~ canopy_c, sp_UMF, method="SANN")
      fm8 <- occu(~ elev ~ basal_a, sp_UMF, method="SANN")
      fm9 <- occu(~ elev ~ in_out, sp_UMF, method="SANN")
      fm10 <- occu(~ basal_a ~ elev, sp_UMF, method="SANN")
      fm11 <- occu(~ basal_a ~ slope, sp_UMF, method="SANN")
      fm12 <- occu(~ basal_a ~ dist_def, sp_UMF, method="SANN")
      fm13 <- occu(~ basal_a ~ dist_rd, sp_UMF, method="SANN")
      fm14 <- occu(~ basal_a ~ canopy_h, sp_UMF, method="SANN")
      fm15 <- occu(~ basal_a ~ canopy_c, sp_UMF, method="SANN")
      fm16 <- occu(~ basal_a ~ basal_a, sp_UMF, method="SANN")
      fm17 <- occu(~ basal_a ~ in_out, sp_UMF, method="SANN")
...
#     
  ...
  ##model list
Cands2 <- list(fm16, fm8,  fm14)
Modnames2 <- c("p(basal_a)psi(in_out)", "p(elev)psi(in_out)", "p(basal_a)psi(canopy_c)" )
...
## El nieque (_Dasyprocta punctata_)
### Matriz de datos colapsada a 15 dias
    
    ```{r mat_sp26,cache=TRUE,warning=FALSE,eval=TRUE, echo=FALSE,error=FALSE, message=FALSE, fig.height=4,fig.width=7}
    f.sp.occu.plot.mat(sp_number = 26)
    ```
    
### Selección de Modelos _Dasyprocta punctata_
    ```{r modeo_sps26,cache=TRUE,warning=FALSE,eval=TRUE, echo=FALSE,error=FALSE, message=FALSE, results='asis'}
...
#### analisis _Dasyprocta punctata_ p(basal_a)psi(dist_rd)  sp=26 
```{r nieque ,cache=FALSE,warning=FALSE,eval=TRUE, echo=FALSE,results='asis'}
...
#     
  ...
  ##########################################   
# ##model list
 Cands2 <- list(fm12, fm4, fm13)
 Modnames2 <- c("p(basal_a)psi(dist_rd)", "p(elev)psi(dist_rd)", "p(basal_a)psi(canopy_h)")
...
pred.data26<-newdata.out
##add predictions to data set to keep everything in the same place
pred.data26$fit <- avg_result[,1]
pred.data26$se.fit <- avg_result[,2]
##compute 95% CI on logit scale, then back transform 
##these asymmetric CI's have better properties
pred.data26$low95 <- plogis(out.preds.logit$mod.avg.pred - 
                          1.96 * out.preds.logit$uncond.se)
pred.data26$upp95 <- plogis(out.preds.logit$mod.avg.pred + 
                          1.96 * out.preds.logit$uncond.se)
...
png(file = "C:/Users/Diego/Documents/CodigoR/Pacoche_unmakerd/fig/p_D_punctata.png", width = 1200, height = 1200, res = 300)
par(mar = c(5, 4, 2, 2) + 0.1)

  plot(fit ~ basal_a, pred.data26, type="l", ylim=c(-0.1,1), col="blue",
       xlab="Basal area",
       ylab="Detection probability",
       xaxt="n")# 
  lines(low95 ~ basal_a, pred.data26, type="l", col=gray(0.5))
  lines(upp95 ~ basal_a, pred.data26, type="l", col=gray(0.5))
 
  xticks <- -1.5:4 # see min(cam.and.covs.scaled$basal_a) #  see wich number equals to var name using: colnames(cam.and.covs)
  xlabs <- xticks*sd(cam.and.covs[,4]) + mean(cam.and.covs[,4]) #Use the mean and sd of original value to change label name
  axis(1, at=xticks, labels=round(xlabs, 1))
##turn off graphics device
dev.off( )
...
png(file = "C:/Users/Diego/Documents/CodigoR/Pacoche_unmakerd/fig/occu_D_punctata.png", width = 1200, height = 1200, res = 300)
par(mar = c(5, 4, 2, 2) + 0.1)

  plot(fit ~ dist_rd, pred.data26, type="l", ylim=c(-0.1,1), col="blue",
       xlab="Distance to road (m)",
       ylab="Occupancy probability",
       xaxt="n")# 
  lines(low95 ~ dist_rd, pred.data26, type="l", col=gray(0.5))
  lines(upp95 ~ dist_rd, pred.data26, type="l", col=gray(0.5))
 
  xticks <- -1:2.9 # see min(cam.and.covs.scaled$dist_rd) #  see wich number equals to var name using: colnames(cam.and.covs)
...
##turn off graphics device
# dev.off( )  
Cands3 <- list(fm12, fm4)
 Modnames3 <- c("p(basal_a)psi(dist_rd)", "p(elev)psi(dist_rd)")
 
############################
# using library("MuMIn")
############################
# get the AICc using the package library("MuMIn")
 library("MuMIn")
 msAICc <- model.sel(models@fits) # makes model selection object
#models with delta.aicc < 4
Average_AICc_2<-model.avg(msAICc, subset = delta < 2) # mod.avg using delta2
psioccu_dasyprocta <- predict(Average_AICc_2, 
                             type="state",
                             newdata=newdata.out,
                             na.rm=TRUE)

# just two first to test predict spatial
Average_AICc_2<-model.avg(msAICc, subset = delta < 0.03) # mod.avg using delta2

# newdata.cov <- data.frame(basal_a=0,
#                           elev=0,
#                       dist_rd=as.vector(cov.stack.standarized[[4]]))
# 
# # creates new raster layers for canopy and basal area in mean=0 
# map0<-cov.stack.standarized[[1]]
# map0[]<-0
# map0@data@names<-"canopy_h"
# map1<-cov.stack.standarized[[1]]
# map1[]<-0
# map1@data@names<-"basal_a"
# s <- addLayer(cov.stack.standarized, map0, map1)
# 
# psioccu_dasyprocta2 <- predict(fm12, #Average_AICc_2, # does not work using average
#                              type="state",
#                              newdata=s)#,
#                              # na.rm=FALSE)
# 
# psioccu_dasyprocta3<-modavgPred(cand.set = Cands3, 
#                      modnames = Modnames3, 
#                      newdata = cov.stack.standarized, 
#                      parm.type = "psi", # "psi"), #detect"), # or "detect"
#                      type = "link")
# 
# map<-cov.stack.standarized[[1]] # copy elev format on map
# map[]<-psioccu_dasyprocta2$fit # drape occ_Tapir on map
# map@data@names<-"fit" # change name
# Tapir<-map
# 
# map1<-addLayer(cov.stack.standarized, psioccu_dasyprocta$fit)
# map1$fit<-psioccu_dasyprocta$fit
# ############################ 
# ########## map
# ############################ 
#   
# dist_rd_map.sd<- cellStats(dist.road.m, 'sd') 
# dist_rd_map.mean<- cellStats(dist.road.m, 'mean')
# g.dist_rd_map<-(dist.road.m - dist_rd_map.mean)/dist_rd_map.sd #map Scaled by sd
# s <- raster(nrow=600, ncol=400) # remustrea a mas chico
# s <- resample(g.dist_rd_map, s, method='bilinear')
# dist_rd.stack<-stack(g.dist_rd_map)
# names(dist_rd.stack)<-"dist_rd"
# psioccu_cuniculus <- predict(fm12, 
#                              type="state",
#                              newdata=cov.stack.standarized
#                              )# type puede # take a while patience it is a large file!!! 10 x 10 m = 70 Megas
# # output.psi <- modavgPred(cand.set = Cands3, 
# #                      modnames = Modnames3, 
# #                      newdata = cov.stack.standarized, 
# #                      parm.type = "psi", #"det"), # or "psi" 
# #                      type = "response")
...
```{r leopardalis ,cache=FALSE,warning=FALSE,eval=TRUE, echo=FALSE,results='asis'}
    ...
    #######################
     ...
     #  covariates of detection and occupancy in that order.
      fm0 <- occu(~ 1 ~ 1, sp_UMF, method="SANN") 
      fm1 <- occu(~ elev ~ elev, sp_UMF, method="SANN")
      fm2 <- occu(~ elev ~ slope, sp_UMF, method="SANN")
      fm3 <- occu(~ elev ~ dist_def, sp_UMF, method="SANN")
      fm4 <- occu(~ elev ~ dist_rd, sp_UMF, method="SANN")
      fm5 <- occu(~ elev ~ canopy_h, sp_UMF, method="SANN")
      fm6 <- occu(~ elev ~ canopy_c, sp_UMF, starts=c(100,200,3,4), method="BFGS")
      fm7 <- occu(~ elev ~ basal_a, sp_UMF, method="SANN")
      fm8 <- occu(~ elev ~ in_out, sp_UMF, method="SANN")
      fm9 <- occu(~ basal_a ~ elev, sp_UMF, method="SANN")
      fm10 <- occu(~ basal_a ~ slope, sp_UMF, method="SANN")
      fm11 <- occu(~ basal_a ~ dist_def, sp_UMF, method="SANN")
      fm12 <- occu(~ basal_a ~ dist_rd, sp_UMF, method="SANN")
      fm13 <- occu(~ basal_a ~ canopy_h, sp_UMF, method="SANN")
      fm14 <- occu(~ basal_a ~ canopy_c, sp_UMF,  starts=c(100,200,3,4), method="BFGS")
      fm15 <- occu(~ basal_a ~ basal_a, sp_UMF, method="SANN")
      fm16 <- occu(~ basal_a ~ in_out, sp_UMF, method="SANN")
...
# # To obtain the coeficient given a model-averaged estimate of covariates
# modavg(cand.set=Cands2, parm="basal_a", modnames=Modnames2, second.ord = TRUE, nobs = NULL, uncond.se = "revised", conf.level = 0.95, 
#        exclude = NULL, warn = F, c.hat = 1, parm.type = "detect")
...
png(file = "C:/Users/Diego/Documents/CodigoR/Pacoche_unmakerd/fig/p_L_pardalis.png", width = 1200, height = 1200, res = 300)
par(mar = c(5, 4, 2, 2) + 0.1)
  ...
  xticks <- -2:4 # see min(cam.and.covs.scaled$elev) #  see wich number equals to var name using: colnames(cam.and.covs)
  xlabs <- xticks*sd(cam.and.covs[,4]) + mean(cam.and.covs[,4]) #Use the mean and sd of original value to change label name
  axis(1, at=xticks, labels=round(xlabs, 1))
##turn off graphics device
dev.off( )
...
png(file = "C:/Users/Diego/Documents/CodigoR/Pacoche_unmakerd/fig/occu_L_pardalis.png", width = 1200, height = 1200, res = 300)
par(mar = c(5, 4, 2, 2) + 0.1)

  plot(fit ~ canopy_c, pred.data26, type="l", ylim=c(-0.1,1), col="blue",
       xlab="Canopy cover",
       ylab="Occupancy probability",
       xaxt="n")# 
  lines(low95 ~ canopy_c, pred.data26, type="l", col=gray(0.5))
  lines(upp95 ~ canopy_c, pred.data26, type="l", col=gray(0.5))
...
## Ardilla (_Sciurus stramineus_)
### Matriz de datos colapsada a 15 dias
    
    ```{r mat_sp30,cache=TRUE,warning=FALSE,eval=TRUE, echo=FALSE,error=FALSE, message=FALSE, fig.height=4,fig.width=7}
    f.sp.occu.plot.mat(sp_number = 30)
    ```
    
### Selección de Modelos _Sciurus stramineus_
    ```{r modeo_sps30,cache=TRUE,warning=FALSE,eval=TRUE, echo=FALSE,error=FALSE, message=FALSE, results='asis'}
    print(as.character(sp.names[sp_number=30]))
    f.sp.occu.models(sp_number = 30)
    ```
    
    

#### analisis _Sciurus stramineus_p(canopy_c)psi(slope)   sp=30 
```{r Sstramineus ,cache=FALSE,warning=FALSE,eval=TRUE, echo=FALSE,results='asis'}
    ...
    #######################
     ...
     #  covariates of detection and occupancy in that order.
      fm0 <- occu(~ 1 ~ 1, sp_UMF, starts=c(15,33), method="SANN") 
      fm1 <- occu(~ elev ~ elev, sp_UMF, method="SANN")
      fm2 <- occu(~ elev ~ slope, sp_UMF, starts=c(100,200,3,4), method="BFGS")
      fm3 <- occu(~ elev ~ dist_def, sp_UMF, method="SANN")
      fm4 <- occu(~ elev ~ dist_rd, sp_UMF, method="SANN")
      fm5 <- occu(~ elev ~ canopy_h, sp_UMF, method="SANN")
      fm6 <- occu(~ elev ~ canopy_c, sp_UMF, starts=c(100,200,3,4), method="BFGS")
      ...
      fm13 <- occu(~ basal_a ~ canopy_h, sp_UMF, method="SANN")
      fm14 <- occu(~ basal_a ~ canopy_c, sp_UMF,  starts=c(100,200,3,4), method="BFGS")
      fm15 <- occu(~ basal_a ~ basal_a, sp_UMF, starts=c(13,245,35,42), method="BFGS")
      fm16 <- occu(~ basal_a ~ in_out, sp_UMF, method="SANN")
...
predictions1minn = data.frame(x = dataminn.pr$ExactSTminn*sd.ndviminn + mean.ndviminn,##log para back transform ?
                          y = dataminn.pr$WaterDistSTminn*sd.WaterDistminn + mean.WaterDistminn,##log para back transform ?
                          #z = predichos$Predicted
                          z = abs(xyzminn[,3])) 
elevationminn.loess = loess(z ~ x*y, data = predictions1minn, degree = 2) #arma el grid 3d


elevationminn.fit = expand.grid(list(x = seq(3763, 9242.212, 10), y=seq(9.950435, 3213.035, 10)))
z = predict(elevationminn.loess, newdata = elevationminn.fit)
elevationminn.fit$Height = as.numeric(z)
library("scales")
require(grDevices)
mycolors<-colorRampPalette(c("red", "white", "blue"))
mycolors<-colorRampPalette(brewer.pal(11, 'RdYlGn')[2:10]) 

levelplot(Height ~ x*y, data = elevation.fit,
          xlab = "NDVI", ylab = "Distance to water",
          main = "Surface Model abundance(NDVI*Distance to Water)",
