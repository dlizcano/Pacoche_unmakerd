...
#  
    ...
    f.sp.occu.models <- function(sp_number){
      ...
      fm3 <- occu(~ 1 ~ dist_def, sp_UMF)
      fm4 <- occu(~ 1 ~ dist_rd, sp_UMF)
      fm5 <- occu(~ 1 ~ canopy_h, sp_UMF)
      fm6 <- occu(~ 1 ~ canopy_c, sp_UMF)
      fm7 <- occu(~ 1 ~ basal_a, sp_UMF)
      fm8 <- occu(~ 1 ~ in_out, sp_UMF)
      fm9 <- occu(~ elev ~ elev, sp_UMF)
      fm10 <- occu(~ elev ~ slope, sp_UMF)
      fm11 <- occu(~ elev ~ dist_def, sp_UMF)
      fm12 <- occu(~ elev ~ dist_rd, sp_UMF)
      fm13 <- occu(~ elev ~ canopy_h, sp_UMF)
      fm14 <- occu(~ elev ~ canopy_c, sp_UMF)
      fm15 <- occu(~ elev ~ basal_a, sp_UMF)
      fm16 <- occu(~ elev ~ in_out, sp_UMF)
      fm17 <- occu(~ basal_a ~ elev, sp_UMF)
      fm18 <- occu(~ basal_a ~ slope, sp_UMF)
      fm19 <- occu(~ basal_a ~ dist_def, sp_UMF)
      fm20 <- occu(~ basal_a ~ dist_rd, sp_UMF)
      fm21 <- occu(~ basal_a ~ canopy_h, sp_UMF)
      fm22 <- occu(~ basal_a ~ canopy_c, sp_UMF)
      fm23 <- occu(~ basal_a ~ basal_a, sp_UMF)
      fm24 <- occu(~ basal_a ~ in_out, sp_UMF)
      ...
      models <- fitList(
        ...
        'p(elev)psi(dist_def)' = fm6,
        'p(basal_a)psi(.)' = fm7,
        'p(basal_a)psi(elev)' = fm8,
        'p(basal_a)psi(canopy_c)' = fm9,
        'p(.)psi(canopy_c)' = fm10,
        'p(.)psi(in_out+elev)' = fm11,
        'p(.)psi(in_out+dist_def)' = fm12
      )
...
```{r cuniculus, cache=FALSE,warning=FALSE,eval=TRUE, echo=FALSE,results='asis'}
    ...
    # fm17 <- occu(~ canopy_c ~ dist_def+in_out, sp_UMF, starts = c(0.1,0.5,0.7,0.8,1))
      ...
      # 'p(canopy_c)psi(dist_def+in_out)' = fm17
      )
